...
func loadClusterMetadataLogs(paths []string) error {
	// Rebuild metadata map on each reload to avoid serving stale partial entries.
...
// lookupTopicMetadataFromLogs is a tolerant fallback for DescribeTopicPartitions:
...
func lookupTopicMetadataFromLogs(paths []string, topicName string) (*TopicMetadata, bool, error) {
	...
	// Pass 2: collect all partitions for the discovered topic UUID.
